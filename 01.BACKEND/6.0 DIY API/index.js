...
app.get("/joke/:id",(req,res)=>{
  ...
  const foundjoke=jokes.find((joke)=>joke.id===index)
  res.json(foundjoke);
})
...
app.get("/filter",(req,res)=>{
  ...
  const typejoke=jokes.filter((joke)=>joke.type===joketype);
  res.json(typejoke);
...
app.put("/jokes/:id",(req,res)=>{
  ...
  const putjoke=jokes.find((joke)=>joke.id===putid);
  putjoke.jokeText=req.body.text;
  putjoke.jokeType=req.body.type;
  res.json(putjoke);
})
...
app.delete("/all",(req,res)=>{
  ...
  if(userKey==masterKey){
    ...
    res
    .status(400)
    .json({error:"Your key is in correct"})
  }
});
